CREATE OR REPLACE PROCEDURE p_ejud_mov_decisao AS
    ...
    CURSOR c_movimentos IS
        SELECT MOV.CODDOC, MOV.DTHRMOV,'DECISAO' AS TIPOMOV, F.codfaseext AS CODFASE, MOV.CODCOMPL1, MOV.CODCOMPL2, MOV.CODCOMPL3, NULL AS CODLOCAL, SYSDATE AS DTETL
		...
		AND f.codfaseext IN (3,193,60003,50193) AND MOV.CODCOMPL2 NOT IN (198,200,871);
	
BEGIN
    DBMS_OUTPUT.PUT_LINE('Iniciando a Procedure p_ejud_mov_decisao: ' || TO_CHAR(SYSDATE, 'YY-MM-DD HH24:MI:SS'));
   
   	SELECT NVL(MAX(DTHRMOV), TO_DATE('1900-01-01', 'YYYY-MM-DD'))
    INTO max_dthrmov
    FROM EJUD_MPM_BASEDEDADOS 
    WHERE tipomov = 'DECISAO';
    ...
    DBMS_OUTPUT.PUT_LINE('Finalizando a Procedure p_ejud_mov_decisao: ' || TO_CHAR(SYSDATE, 'YY-MM-DD HH24:MI:SS'));
   
   INSERT INTO mpm_log_execucao (nome, data_execucao) VALUES ('P_EJUD_MOV_DECISAO', SYSTIMESTAMP);
